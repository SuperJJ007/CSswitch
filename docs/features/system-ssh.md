...
`reuse_system_ssh` 默认关闭。关闭时，CSSwitch 不把真实系统 SSH 配置或宿主 `SSH_AUTH_SOCK` 注入隔离 Science。
...
参数仍由调用方交给系统 `ssh`；wrapper 只固定配置文件入口，不实现 SSH 协议，也不读取或显示私钥内容。Linux wrapper 与 runtime 脚本固定使用 `/bin/bash`，不从 PATH 选择 shell。
...
- 不启动 `sshd`，不开启 macOS Remote Login 或任何 Linux SSH server；
...
启用后的每次启动都会再次校验 config 与 packaged wrapper。config 缺失、含符号链接、wrapper 缺失或路径不安全时，启动 fail closed 并清理部分启动，不能以 warning 略过。Linux 只在宿主 `SSH_AUTH_SOCK` 存在、为绝对路径且确认为 Unix socket 时转交 agent；无效 socket 同样阻止启动。只有 Science 已成功启动后的某次 `/usr/bin/ssh` 命令失败，才只影响该命令。
...
4. 默认不继承 agent；opt-in 只接受有效 Unix socket；
