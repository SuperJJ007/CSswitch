...
2. 否则使用平台受信任安装路径：macOS 为 `/Applications/Claude Science.app/Contents/Resources/bin/claude-science`，Linux 为外层真实 `$HOME/.local/bin/claude-science`。
3. 只有平台安装不可用、`<data-dir>/bin/claude-science` 可执行且版本可确认时，preflight 才返回 `cached_choice_required`；用户可授权 `cached_once`。
4. cache 授权只在本次启动的内存中生效，不写成偏好。未知版本或缺失 cache 不可启动。

只要平台安装可用，它始终优先于 cache。CSSwitch 不从不受控 PATH 猜测 binary，不下载 Science、不调用 updater、不覆盖 cache，也不从真实 `~/.claude-science` 复制 `bin`、`conda`、`runtime` 或 `seed-assets`。

Linux 在选择 binary 前还必须通过环境 preflight：x86_64、Bubblewrap 0.8.0+、实际可创建非特权 user namespace、`socat` 与 `lsof`。失败返回 `environment_blocked` 和有界 blocker code；不能转成 warning，也没有 no-sandbox 旁路。
...
Gateway 同样只监听 loopback。桌面和 Gateway 的 browser opener 在 Linux 固定为 `/usr/bin/xdg-open`，macOS 固定为 `/usr/bin/open`；opener 失败不停止已经健康的服务，前端只显示无 query/nonce 的脱敏 loopback 地址并允许重试，重试时由后端重新获取新 URL。exact URL 只在后端内存中交给 opener，URL/nonce 不进入前端、clipboard 或普通日志。端口健康不等于身份健康；公共网络暴露不属于当前合同。
...
用户更新平台安装的 Claude Science 后，下一次 stopped-to-started 启动使用新 executable，同时继续复用原 CSSwitch data-dir。CSSwitch 不迁移或覆盖组织、项目和 Skill 数据。
...
一次上游版本测试不能推出通用最低版本。观察接口变化时，应只关闭受影响 bridge 并如实报告，而不是替换或降级用户安装。
